...
const makeFakeRequest = (): HttpRequest => ({
  body: {
    ...
    passwordConfirmation: 'any_passaword'
  }
})
interface SutTypes {
...
const makeSut = (): SutTypes => {
  ...
  const sut = new SignUpController(emailValidatorStub, addAccountStub, validateStub)
  return {
    sut,
    emailValidatorStub,
    addAccountStub,
...
describe('Signup Controller', () => {
  ...
  test('Should return 400 if Validation returns an error', async () => {
    const { sut, validateStub } = makeSut()
    jest.spyOn(validateStub, 'validate').mockReturnValueOnce(new MissingParamError('any_field'))
    const httpResponse = await sut.handle(makeFakeRequest())
